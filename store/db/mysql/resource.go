...
func (d *DB) ListResources(ctx context.Context, find *store.FindResource) ([]*store.Resource, error) {
	...
	fields := []string{"`id`", "`resource_name`", "`filename`", "`external_link`", "`type`", "`size`", "`creator_id`", "UNIX_TIMESTAMP(`created_ts`)", "UNIX_TIMESTAMP(`updated_ts`)", "`internal_path`", "`memo_id`", "`storage_id`"}
	if find.GetBlob {
